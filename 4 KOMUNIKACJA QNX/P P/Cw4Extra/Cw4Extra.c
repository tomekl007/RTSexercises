	...
	while (1) {
		...
		if (reply.typ == 1) {
			...
			for (i = reply.pocz; i < reply.kon; i++) {
				...
				int prime = 1;
				for (c = 2; c <= i / 2; c++) {
					if (i % c == 0) {//nie moze byc podzielna
						prime = 0;
						break;
					}
				}
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		if (running_threads < threads_count && !f) {
			//f jest po to ze jak na koncu else dam running_threads to weszedl by
			//z powrotem do tego if`a
			pthread_create(&tid, NULL, count_primes2, NULL);
			running_threads++;
			printf("in if\n");
			//jak juz stworze wystaczajaca ilos watkow to przechodze dalej
			//continue;
		} else {
			printf("in first else\n");
			f = 1;//po tym jak juz zmienilem f nie ma prawa wejsc do if(tego na gorze)
			par_t args;
