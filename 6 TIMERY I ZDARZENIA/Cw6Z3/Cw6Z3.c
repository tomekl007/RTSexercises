...
void dodaj(listNode **lista, int y,int t)
{
	listNode *e, *tmp;
     ...
     e->scoid=y;
     e->timeout = t;
     printf("dodaj : e->scoid = %d, timeout : %d\n",e->scoid, e->timeout);
     //atomic_add(&count, 1);
...
int client() {
   ...
   // msg.data = 0;
    ...
    for (msg.data=0; msg.data < 5; msg.data++) {
       ...
       // sleep(2);
        msg.timeout = 1;//timeout jaki bedzie ustawiony w timerTimeout
        sleep(msg.data);
...
int server() {
   ...
   while (1) {
	   event.sigev_notify = SIGEV_UNBLOCK;// a zmienna event (struktura sigevent) określa
	                                    //rodzaj zdarzenia, które zajdzie w przypadku przeterminowani

	   //to powiino byc ustawiane pulsem ktory przydzie od clienta
	   //przed zaczeciem wysylania wiadomosci
	   const uint64_t timeout = 3000000000;//Zmienna timeout zawiera czas przeterminowania w nanosekundach
	   printf("timeout set to : %d", msg.timeout);
	   TimerTimeout( CLOCK_REALTIME, _NTO_TIMEOUT_RECEIVE,&event, &timeout, NULL );
       rcvid = MsgReceive(attach->chid, &msg, sizeof(msg), &info);
      // timeout = msg.timeout;

       //signal( SIGEV_UNBLOCK, MyHandler );
...
int flag;
       if (rcvid == -1) {/* Error condition, exit */
    	   printf("blad przekroczenie timeout przez : %d", info.pid);
    	   printf("->>>>>>> info.pid : %d", info.pid);
    	   kill(info.pid,SIGKILL);
    	 // execl("Cw6Z3", NULL);
    	   spawnl( P_NOWAIT, "Cw6Z3","c", NULL );
flag = 0;
           //break;
       }
       ...
       printf("Server receive %d from : %d \n", msg.data, info.pid);
       ...
       dodaj(&listNode, info.scoid, msg.timeout);
